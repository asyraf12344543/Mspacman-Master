...
/**
 * This class contains the enumerations for the moves and the ghosts as well as as the constants of the game. If you
 * should change the constants, bear in mind that this might significantly affect the game and hence the performance of
 * your controller. The set of constants used in the competition may be found on the competition website.
 */
public final class Constants {
	/**
	 * Enumeration for the moves that are possible in the game. At each time step, a controller is required to supply
	 * one of the 5 actions available. If the controller replies NEUTRAL or does not reply in time, the previous action
	 * is repeated. If the previous action is not a legal move, a legal move is chosen uniformly at random.
	 */
	public enum MOVE {
		UP {
	...
	public static final int GHOST_EAT_SCORE = 200; // score for the first ghost eaten (doubles every time for the
													// duration of a single power pill)
	public static final int EDIBLE_TIME = 200; // initial time a ghost is edible for (decreases as level number
												// increases)
	public static final float EDIBLE_TIME_REDUCTION = 0.9f; // reduction factor by which edible time decreases as level
															// number increases
	public static final float LAIR_REDUCTION = 0.9f; // reduction factor by which lair times decrease as level number
														// increases
	...
	public static final int AWARD_LIFE_LEFT = 800; // points awarded for every life left at the end of the game (when
													// time runs out)

	public static final int EAT_DISTANCE = 2; // distance in the connected graph considered close enough for an eating
												// event to take place
	...
	public static final int GHOST_SPEED_REDUCTION = 2; // difference in speed when ghosts are edible (every
														// GHOST_SPEED_REDUCTION, a ghost remains stationary)
	public static final int EDIBLE_ALERT = 30; // for display only (ghosts turning blue)
	public static final int INTERVAL_WAIT = 1; // for quicker execution: check every INTERVAL_WAIT ms to see if
												// controllers have returned
